...
def render_run_inspector(df, runs_list):
    ...
    for r in runs_list:
        ...
        if r['run_type'] == "DHW":
            icon = "🚿"
            # Use .get() for optional analysis keys
    ...
    c3.metric("Avg ΔT", f"{selected_run.get('avg_dt', 0):.1f}°")
    ...
    with tab1:
        ...
        # Use Heat_Clean/Power_Clean if processed, otherwise fallback
    ...
    with tab2:
        if selected_run['run_type'] == "DHW":
            # These might not be calculated in processing.py yet, use defaults
    ...
    with tab3:
        if selected_run['run_type'] == "Heating":
            ...
            # Identify relevant room sensors dynamically
            room_cols = [c for c in run_data.columns if c.startswith('Room_')]
            deltas = selected_run.get('room_deltas', {})
            
            # If explicit "relevant_rooms" exists (from zones), prefer those
