...
def process_uploaded_files_once(uploaded_files):
    ...
    # Cache everything including unmapped entities
    cache = {
        "file_key": file_key, 
        "df": df, 
        "runs": runs_list, 
    ...
    if show_inspector:
        ...
        for fname, details in file_details.items():
            with st.expander(f"📄 {fname} ({details.get('structure', 'Error')})"):
                if "error" in details:
                    st.error(f"Read Error: {details['error']}")
                else:
                    c1, c2 = st.columns(2)
                    with c1:
                        st.markdown("**Raw Columns:**")
                        st.code("\n".join(details['columns_raw']), language="text")
                    with c2:
                        st.markdown(f"**Entities Detected ({len(details['entities_found'])}):**")
                        if details['entities_found']:
            ...
            # ==========================================
                ...
                if runs_list:
                    ...
                    with tab2:
                        ...
                        fig2 = make_subplots(
                            rows=num_rows, cols=1, shared_xaxes=True, vertical_spacing=0.08,
                            subplot_titles=row_titles, specs=[[{"secondary_y": False}]] * num_rows
                        )
            ...
            # ==========================================
                ...
                if not daily_df.empty:
                    ...
                    with dq_tab3:
                        ...
                        if count_cols:
                            ...
                            for c in count_cols:
                                ...
                                if mode == 'event_only': flat_data[clean_name] = daily_df[c].fillna(0).astype(int)
                                else: flat_data[clean_name] = availability_pct(daily_df[c], expected_window_series(clean_name, system_on_minutes)).round(0)
                            ...
                            event_cols = [col for col in df_final.columns if SENSOR_EXPECTATION_MODE.get(col[1], 'system') == 'event_only']
                            normal_cols = [col for col in df_final.columns if col not in event_cols]

                            styler = df_final.style.format("{:.0f}", na_rep="-")
                            if normal_cols: styler = styler.background_gradient(subset=normal_cols, cmap='RdYlGn', vmin=0, vmax=100)
                            if event_cols: styler = styler.map(lambda x: "background-color: #e0e0e0; color: #555555", subset=event_cols)
                    ...
                    with dq_tab5:
                        ...
                        st.caption("These sensors were found in your files but are not defined in `ENTITY_MAP` in `config.py`. They are preserved here for inspection.")
                        
                        if unmapped_entities:
                            # 1. Show simple list
                            with st.expander("See List of Names", expanded=False):
                                st.code("\n".join(unmapped_entities), language="text")

                            # 2. Show Stats Table
                            # We access the raw 'df' from cache to calculate fresh stats for these specific columns
                            unmapped_stats = []
                                ...
                                if entity in df.columns:
                                    ...
                                    if non_null > 0:
                                        unmapped_stats.append({
                                            "Entity ID": entity,
                                            "Samples": non_null,
                                            "First Seen": series.dropna().index.min().strftime('%d/%m %H:%M'),
                                            "Last Seen": series.dropna().index.max().strftime('%d/%m %H:%M'),
                                            "Mean Value": series.mean() if pd.api.types.is_numeric_dtype(series) else "N/A"
                                        })
                            
                            if unmapped_stats:
                                st.dataframe(pd.DataFrame(unmapped_stats).set_index("Entity ID"), width="stretch")
                            else:
                                st.warning("Unmapped entities found in list, but no data present in dataframe (columns might have been dropped during merge).")
                        else:
                            st.success("No unmapped entities found. Your config covers 100% of the data!")
