...
def render_data_quality(daily_df, df, unmapped_entities, patterns, heartbeat_path):
    ...
    # --- PARTIAL DAY LOGIC ---
    # We calculate the denominator based on actual recorded minutes
    system_on_minutes = daily_df.apply(lambda r: max(r.get('Recorded_Minutes', 0), r.get('DQ_Power_Count', 0), r.get('Power_count', 0), 1), axis=1)
    
    def expected_window_series(sensor_name, system_on_minutes):
        # 1. Check if we have a baseline
        baseline_data = None
        if "heartbeat_baseline" in st.session_state and st.session_state["heartbeat_baseline"]:
             baseline_data = st.session_state["heartbeat_baseline"].get(sensor_name)
    ...
    def format_dq_df(df_in):
        ...
        try:
            ...
            pass # Keep original if not datetime
        return df_out
    ...
    with dq_tab1:
        ...
        for group_name, sensors in SENSOR_GROUPS.items():
            ...
            # Identify valid columns in daily_df (processing produces {Sensor}_count usually)
            valid_sensors = []
            for s in sensors:
                if f"DQ_{s}_Count" in daily_df.columns: valid_sensors.append(s)
                elif f"{s}_count" in daily_df.columns: valid_sensors.append(s)
    ...
    with dq_tab4:
        ...
        if st.button("Generate Heartbeat Baseline"):
            history_df = st.session_state.get("raw_history_df") # Should be passed from app.py
            if history_df is None and "cached" in st.session_state:
                # Fallback if raw_history not explicitly passed but in cache
                # Note: app.py doesn't currently put raw_history in "cached" dictionary
                # It returns it from loader but only caches 'df', 'runs', 'daily', 'patterns'
                st.warning("Raw history not available in current session context. Reload data.")
            else:
                # If you modify app.py to pass raw_history, this works.
                # Assuming app.py might have stored it elsewhere or we skip for now.
                st.info("Baseline generation requires raw history access.")
