...
def render_long_term_trends(daily_df, raw_df, runs_list):
    ...
    with tab_main:
        ...
        total_heat = daily_df['Total_Heat_kWh'].sum()
        total_elec = daily_df['Total_Electricity_kWh'].sum()
        total_cost = daily_df['Daily_Cost_Euro'].sum()
        ...
        if 'Heat_DHW_kWh' in daily_df.columns:
            fig.add_trace(go.Bar(
                ...
                base=daily_df.get('Heat_Heating_kWh', 0), legendgroup='Output',
        ...
        base_elec = daily_df.get('Electricity_Heating_kWh', 0)
        ...
        if 'Electricity_DHW_kWh' in daily_df.columns:
            fig.add_trace(go.Bar(
                ...
                base=base_elec, legendgroup='Input',
                hovertemplate=dhw_elec_hover
            ))
            base_elec += daily_df['Electricity_DHW_kWh']
